...
| Item                          | Détail                                                                                                    |
| ----------------------------- | --------------------------------------------------------------------------------------------------------- |
| **GLSL header vs `layout(binding=)`** | Le translator émet `#version 410 core` puis utilise `layout(std140, binding=N)`. **`glslangValidator` strict rejette** : "'binding' not supported for this version or the enabled extensions". Les drivers OpenGL desktop (Mesa/NVIDIA/AMD/Intel modernes) sont **laxistes** et l'acceptent, ce qui explique pourquoi le code marche en pratique. Deux corrections possibles, à trancher après capture réelle : (a) ajouter `#extension GL_ARB_shading_language_420pack : require`, ou (b) monter à `#version 420 core`. `REQUIRES WINDOWS/GPU VALIDATION`. |
